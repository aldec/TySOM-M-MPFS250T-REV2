...
volatile struct can_device * dev1;
int uiofd, uiofd1;
...
uint32_t getMemSize( char* sysfs_path, char* device_path){
	uint32_t mmap_size = get_memory_size(sysfs_path, device_path);
	if (mmap_size == 0) {
...
void* listener(void* p){
	printf("waiting for interrupt in listener\n");
    int ret=0;
    while(ret<1){
            ret = wait_for_interrupt(uiofd1, dev1);
...
int main(int argc, char *argv[])
{
	...
	char sysfs_path[SYSFS_PATH_LEN];
	uint32_t mmap_size;
	char uio_device1[UIO_DEVICE_PATH_LEN];
	char sysfs_path1[SYSFS_PATH_LEN];
	uint32_t mmap_size1;
	struct mss_can_filter filter;
	struct can_msg msg;
	uint32_t bitrate = CAN_SPEED_8M_20K;
	...
	index = 2;
	snprintf(uio_device1, UIO_DEVICE_PATH_LEN, "/dev/uio%d", index);
	printf("located %s\n", uio_device1);
	for (i=0; i<500000; ++i)
		;
	uiofd = openCan(uio_device1);
	if ((uiofd<0))	{
		return -1;
	}
	index = 1;
	...
	for (i=0; i<500000; ++i)
		;
	snprintf(sysfs_path, SYSFS_PATH_LEN, sysfs_template, 0, "maps/map0/size");
	mmap_size = getMemSize(sysfs_path, uio_device);
	if ((mmap_size == 0)) {
		return -1;
	}
	snprintf(sysfs_path1, SYSFS_PATH_LEN, sysfs_template, 1, "maps/map0/size");
	mmap_size1 = getMemSize(sysfs_path1, uio_device1);
	if ((mmap_size1 == 0)) {
		return -1;
	}
	...
	dev1 = mmap(NULL, mmap_size1, PROT_READ|PROT_WRITE, MAP_SHARED, uiofd1, 0);
	if (dev == MAP_FAILED) {
		fprintf(stderr, "cannot mmap %s: %s\n", uio_device1, strerror(errno));
		return -1;
	} else {
		printf("mapped 0x%x bytes for %s\n", mmap_size1, uio_device1);
	}
	for (i=0; i<500000; ++i)
		;

	printf("setting can device at %s into normal mode\n", uio_device);
	for (i=0; i<500000; ++i)
		;
	...
	for (i=0; i<500000; ++i)
		;
	can_init(dev1, bitrate);
	can_set_mode(dev1, CAN_MODE_NORMAL);

  	printf("Start devices\n");
	can_start(dev);
	can_start(dev1);
	...
	can_cfg_buf(dev1, &filter);

	/* enable interrupts */
	can_set_int_enb(dev, int_enb);
	can_set_int_enb(dev1, int_enb);
    ...
    printf("MSG sent, waiting for interrupt\n");
	while(wait_for_interrupt(uiofd, dev))
		;

	printf("Wait for thread to join\n");
  	pthread_join(id, (void**)&ptr);
	ret = munmap((void *)dev, mmap_size);
	printf("unmapped %s\n", uio_device);
	ret = close(uiofd);
	printf("closed %s\n", uio_device);
	ret = munmap((void *)dev1, mmap_size1);
	printf("unmapped %s\n", uio_device1);
	ret = close(uiofd1);
	printf("closed %s\n", uio_device1);
	return ret;
