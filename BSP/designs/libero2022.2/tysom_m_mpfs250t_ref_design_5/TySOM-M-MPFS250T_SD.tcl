#TySOM-M-MPFS250T SD Libero design
...
download_core -vlnv {Actel:SystemBuilder:PF_DDR4:2.5.108} -location {www.actel-ip.com/repositories/SgCore} 
...
source ./script_support/TySOM_M_MPFS250T_SD_recursive.tcl
...
import_files \
         ...
         -io_pdc "${constraint_path}/ALDEC_CAN1.pdc" \
		 ...
		 -io_pdc "${constraint_path}/ALDEC_SDIO.pdc" \
		  -io_pdc "${constraint_path}/ALDEC_FMC2.pdc" \
		 -io_pdc "${constraint_path}/ALDEC_DDR4.pdc"  

organize_tool_files -tool {PLACEROUTE} -file "${project_dir}/constraint/io/ALDEC_I2C0.pdc" -file "${project_dir}/constraint/io/ALDEC_CAN1.pdc" -file "${project_dir}/constraint/io/ALDEC_SDIO.pdc" -file "${project_dir}/constraint/io/ALDEC_USB.pdc" -file "${project_dir}/constraint/io/ALDEC_MAC.pdc" -file "${project_dir}/constraint/io/ALDEC_UARTs.pdc" -file "${project_dir}/constraint/ALDEC_SDC.sdc" -file "${project_dir}/constraint/io/ALDEC_DDR4.pdc" -file "${project_dir}/constraint/io/ALDEC_FMC2.pdc" -module {tysom_m_2022_2_SD::work} -input_type {constraint} 
organize_tool_files -tool {VERIFYTIMING} -file "${project_dir}/constraint/ALDEC_SDC.sdc" -module {tysom_m_2022_2_SD::work} -input_type {constraint} 
organize_tool_files -tool {SYNTHESIZE}  -file "${project_dir}/constraint/ALDEC_SDC.sdc" -module {tysom_m_2022_2_SD::work} -input_type {constraint} 
