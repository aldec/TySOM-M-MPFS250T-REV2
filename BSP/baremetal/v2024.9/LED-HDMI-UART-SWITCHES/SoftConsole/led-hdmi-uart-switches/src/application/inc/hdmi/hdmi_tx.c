...
//static i2c_instance_t * hdmi_i2c = &g_i2c_instance_hdmi;

mss_i2c_instance_t *hdmi_i2c = &g_mss_i2c0_lo;
...
static mss_i2c_status_t hdmi_i2c_tx_write(uint8_t i2c_div_address, uint8_t data_reg, uint8_t data)
{
	...
	MSS_I2C_write(hdmi_i2c, i2c_div_address, (const uint8_t *)tx_buffer, write_length, MSS_I2C_RELEASE_BUS );

    status = MSS_I2C_wait_complete( hdmi_i2c, MSS_I2C_NO_TIMEOUT );
	return status;
}

static mss_i2c_status_t hdmi_i2c_tx_read(uint8_t i2c_div_address, uint8_t data_reg, uint8_t *readdata, uint8_t readlen)
{
    mss_i2c_status_t status;
    MSS_I2C_write_read( hdmi_i2c, i2c_div_address, &data_reg, 1, readdata, readlen, MSS_I2C_RELEASE_BUS);
    status = MSS_I2C_wait_complete( hdmi_i2c, MSS_I2C_NO_TIMEOUT );
	return status;
}
void HDMI_tx_init()
{
	...
	printf("start init\n");
	MSS_I2C_init( hdmi_i2c, 0x39, MSS_I2C_PCLK_DIV_256 );
	printf("stop init\n");
	...
	do{
   	...
   	{
		if(hdmi_i2c_tx_write(I2C_HDMI_OUT_ADDR, iic_hdmi_out_config[inc][0], iic_hdmi_out_config[inc][1]) == MSS_I2C_SUCCESS)
    	{
			printf("transaction successful\n");
    	}

 		if(hdmi_i2c_tx_read(I2C_HDMI_OUT_ADDR, iic_hdmi_out_config[inc][0], &read_data, 1) == MSS_I2C_SUCCESS)
   		{
