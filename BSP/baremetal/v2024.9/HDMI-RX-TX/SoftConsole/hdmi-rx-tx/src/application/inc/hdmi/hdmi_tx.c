...
static mss_i2c_status_t hdmi_i2c_tx_write(uint8_t i2c_div_address, uint8_t data_reg, uint8_t data)
{
	...
	MSS_I2C_write(&g_mss_i2c1_lo, i2c_div_address, (const uint8_t *)tx_buffer, write_length, MSS_I2C_RELEASE_BUS );

    status = MSS_I2C_wait_complete(&g_mss_i2c1_lo, MSS_I2C_NO_TIMEOUT );
	return status;
}
...
{
    mss_i2c_status_t status;
    MSS_I2C_write_read(&g_mss_i2c1_lo, i2c_div_address, &data_reg, 1, readdata, readlen, MSS_I2C_RELEASE_BUS);
    status = MSS_I2C_wait_complete(&g_mss_i2c1_lo, MSS_I2C_NO_TIMEOUT );
	return status;
}
void HDMI_tx_init()
{
	...
	do{
   	...
   	uart_printf("ADV7513 initializing done.\r\n");
}
